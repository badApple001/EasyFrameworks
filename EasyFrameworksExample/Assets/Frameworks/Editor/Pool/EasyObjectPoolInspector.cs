...
public class EasyObjectPoolInspector : Editor
{
    ...
    private const string outScriptPath = "Assets/Framework_AutoGenerate/Scripts/Pool";
    private const string settingAssetPath = "Assets/Framework_AutoGenerate/Resources/PoolSettingAsset.asset";
    ...
    private void GenenratePoolObjectPath( )
    {
        if ( null != _asset && _asset.settings.Count > 0 )
        {
            ...
            if ( !Directory.Exists( outScriptPath ) )
            {
                Directory.CreateDirectory( outScriptPath );
            }
            File.WriteAllText( $"{outScriptPath}/PoolObjectPath.cs", sb.ToString( ) );
        }
    }
