...
public class AudioManagerInspector : Editor
{
    ...
    public override void OnInspectorGUI( )
    {
        ...
        {
            SaveAsset( _asset );
            GenerateAudioPathClass( );
        }
    }

    private void GenerateAudioPathClass( )
    {
        if ( null != _asset && _asset.settings.Count > 0 )
        {
            ...
            foreach ( var setting in settings )
            {
                ...
                sb.AppendLine( "\t/// </summary>" );
                sb.AppendLine( $"\tpublic const string {name3} = \"{setting.name}\";" );
                sb.AppendLine( );
            }
            ...
            if ( !Directory.Exists( "Assets/Frameworks/Modules/Audio/Scripts" ) )
            {
                Directory.CreateDirectory( "Assets/Frameworks/Modules/Audio/Scripts" );
            }
            File.WriteAllText( "Assets/Frameworks/Modules/Audio/Scripts/AudioPath.cs", sb.ToString( ) );
        }
    }
    ...
    private static string GetAudioNameByRelativeParentDirectory( string fullPath )
    {
        ...
        {
            string name1 = fullPath.Replace( "Assets/BundleRes/", "" );
            return name1;
        }

        string parent = Path.Combine( Application.dataPath, "Assets/BundleRes/" );
