//==========================
// - FileName:      Assets/Frameworks/Scripts/CatAsset/Runtime/Loader/DefaultAssetLoader.cs
// - Created:       ChenJC	
...
namespace CatAsset.Runtime
{
    ...
    {
        /// <inheritdoc />
        protected override AssetHandler<T> InternalLoadAsset<T>( string assetName, CancellationToken token,
            TaskPriority priority )
        {
            ...
            switch ( category )
            {
                ...
                case AssetCategory.InternalBundledAsset:
                    //加载内置资源
                    CatAssetManager.LoadAssetImmediately( assetName, assetType, handler, assetRuntimeInfo );
                    break;

                case AssetCategory.InternalRawAsset:
                case AssetCategory.ExternalRawAsset:
                    //加载原生资源
                    assetName = RuntimeUtil.GetReadOnlyPath( assetRuntimeInfo.BundleManifest.BundleIdentifyName );
                    CatAssetManager.LoadRawAssetImmediately( assetName, assetType, handler, assetRuntimeInfo );
                    break;
            }
        ...
        private void InternalUnloadAsset( AssetRuntimeInfo assetRuntimeInfo )
        {
            ...
            if ( assetRuntimeInfo.IsUnused( ) )
            {
                ...
                foreach ( var dependency in assetRuntimeInfo.AssetManifest.Dependencies )
                {
                    ...
                    if ( assetRuntimeInfo.BundleManifest.IsScene )
                    {
